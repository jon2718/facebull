...
namespace detail {
    ...
    public:
		...
		//added
		const vector<Edge> &included_edges;
		const vector<Edge> &excluded_edges;
        //
        ...
        OptimumBranching(const TEdgeListGraph &g,
                         ...
                         TInputIterator roots_end,
                         TOutputIterator out)
            : g(g), included_edges(included_edges), excluded_edges(excluded_edges), index(index), weight(weight),
        ...
        void operator()()
        {
            ...
            {
				//std::cout<<"weight is "<<weight[e]<<"\n";  //added
                if (source(e, g) == target(e, g))
                    continue;
				//if (weight[e]==-1)
				
				if (find(excluded_edges.begin(), excluded_edges.end(), e)!=excluded_edges.end())
				{	//std::cout<<"continuing";
					continue;
				}
			...
			BOOST_FOREACH(const Edge&e, included_edges)
			{
            ...
            {
				if (find(single_targets.begin(), single_targets.end(), target(en.edge,g))!=single_targets.end())
				{
					if (find(included_edges.begin(), included_edges.end(), en.edge)!=included_edges.end())
                          ...
                          TOutputIterator out)
{
    ...
    ::detail::OptimumBranching<TOptimumIsMaximum, TAttemptToSpan,
        ...
        TInputIterator, TOutputIterator>
          optimum_branching(g, included_edges, excluded_edges, index, weight, roots_begin, roots_end, out);
    optimum_branching();
}
